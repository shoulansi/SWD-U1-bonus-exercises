...
function itemsIndex(index){
	for (let item in pantry){
    if (Array.isArray(pantry[item]) && pantry[item][index] !== undefined){
		console.log(`Items at index (${index}): \t${pantry[item][index]}`);
	}
	}
...
function checkPackageType(packageType){
	for (let key in pantry){
		if (key.includes(packageType)){
			console.log(`Items that are ${packageType}: -${pantry[key]}`);
		}
	}
};
...
checkPackageType("bottled");
checkPackageType("boxed");
/*
	Great job!
*/
